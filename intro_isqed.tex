...
%Once rectifiability at these nets is ascertained,
...
importance.
% Our approach employs computer algebra
% techniques over finite fields to identify whether the circuit
% admits rectification at a given set of $m$ nets.
% 
% The problem is formulated at a high abstraction level
% using bit-vector words, which allows us to determine the rectifiability 
% of the circuit in an efficient manner.
...
% using quantified Boolean formula solving,
% %~\cite{scholl:2}, or using
% Craig Interpolation (CI), or iterative SAT
% solving~\cite{MF_Huang:DAC11,MF_Huang:DATE12,fujita:2015,SS_Fujita:ISQED17}.  
% %  MF_Roland:ICCAD10,
% Boolean function and SAT-based models are infeasible for rectification 
% of buggy arithmetic circuits. Recent works attempt
% rectification using computer algebra techniques for finite field 
% circuits~\cite{Utkarsh:VLSI18,Vkrao:FMCAD18}, and also for integer
...
% rectification at all. In such cases, the correction has to be
% attempted at multiple targets, say at $m$ distinct nets. This is
...
% where multiple nets are identified as rectification {\it targets}, and
% corresponding rectification patches are computed. 
% Contemporary techniques in multi-fix rectification assume
% availability of target locations.
\par 
...
% the recent approach employs a sampling 
...
% {\it The objective is to decide whether the given set of $m$
%   targets admit multi-fix rectification.} 
%The circuit may or may not be rectifiable at these $m$ targets. This
%MFR-check ascertains whether rectification functions exist that can
